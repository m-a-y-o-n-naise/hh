...
class Base:
    ...
    def open(self, uri) -> Response | None:
        ...
        wait_until='domcontentloaded') #  осуществляет переход на веб-страницу
    ...
    def input_value_by_index(self, locator: str, index: int,
        ...
        self.page.locator(locator).nth(index).fill(data) #  позволяет кликнуть на нужное поле с инпутом, указав его индекс
    ...
    def switch_to_iframe_and_input(self, iframe_locator, locator_for_input,
                                   data: str):  # переключаемся на iframe по локатору и вводим нужные данные
        frame = self.page.frame_locator(iframe_locator)
        if frame is not None:
